...
namespace CloudChat
{
    public static class HandleMethod
    {
        ...
        /// 获取客户端配置项
        /// </summary>
        public static void  ReadXmlConf(string HostName)
        {
            ...
            try
            {
                ...
                if (File.Exists(XMLFilePath + @"\MainEntityData.xml"))
                {
                    ...
                    IEnumerable<XElement> element = from ele in LocalEntity.Elements("FriendEntity")
                                                    where (string)ele.Attribute("name") == HostName
                                                    select ele;
                    if (element.Count() == 1)
                    {
                        ...
                        Program.MainEntity.Staturs = "在线";//上线
            ...
            if (FilePath.Length != 0)
            {
                try
                {
                   ...
                   var NewXel= new XElement("Entity",
                        new XElement("FriendEntity",new XAttribute("name",ComputeName),
